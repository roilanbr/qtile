    ...
    Screen(
        ...
        top=bar.Bar(
            [
                ...
                # Módulo CPU en Gráficos
                fcPuntasWidget(colorOrange, colorPurple, 0),
                # fcIcon("", colorOrange, colorBg),#  nf-fa-microchip
                widget.Memory( background = colorPurple,foreground = colorBg,foreground_alert = colorRed,
                ...
                widget.CheckUpdates(
                    ...
                    no_update_string = " 0 ", #  nf-mdi-autorenew
                    distro = "Arch",
                    mouse_callbacks = { "Botton2": lambda: lazy.cmd_spawn("alacrytty") },
        ...
        bottom=bar.Bar(
            [
                ...
                fcPuntasWidget(colorBg, colorRed, 1),

                widget.TaskList(),
                
                fcPuntasWidget(colorBg, colorPurple, 0),
                widget.Systray(background = colorPurple),

                fcPuntasWidget(colorPurple, colorBg, 0),
                widget.Volume(
                    background = colorBg, fontsize=sizeBar-10, update_interval=0.2, fmt = " {}",
                    mouse_callbacks = {"Button3": lambda: qtile.cmd_spawn("pavucontrol")}),
                
                widget.Clock(
                    foreground=colorGreen,
                    format=" %d/%m/%Y | %I:%M %p", #  nf-mdi-calendar_clock
                    fontsize = sizeBar - 10
                    ),
            ],
            sizeBar,
